...
class FilamentManagerPlugin(FilamentManagerApi,
    ...
    def migrate_database_schema(self, target, current):
        # These database migrations do not work. Alter Table is not supported in sqlalchemy. Need to find
        # alternative. There does not apear any way to handle this programatically. --SFAL
        if current <= 1:
        ...
        if current <=3:
            # Add profile_notes column to profiles table --SFAL
            sql = "Alter TABLE profiles ADD COLUMN profile_notes TEXT NOT NULL DEFAULT '';"
            self.filamentManager.execute_script(sql)
    ...
    def update_pause_thresholds(self):
        def set_threshold(selection):
            def threshold(spool):
                ...
                return length - self._settings.get_float(["pauseThreshold"])
    ...
    def get_update_information(self):
        return dict(
            filamentmanager=dict(
                ...
                # update method: pip
                pip="https://github.com/SireFailsALot/OctoPrint-FilamentManager/archive/{target_version}.zip"
            )
        )
